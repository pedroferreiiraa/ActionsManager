...
public class ProjectController : ControllerBase
{
    ...
    public async Task<IActionResult> GetProjectsDepartment(int departmentId, string search = "", int pageNumber = 1, int pageSize = 10, int status = -1)
    {
        // Incluindo o status na chamada do construtor
        var query = new GetProjectsOfUsersDepartmentQuery(departmentId, search, pageNumber, pageSize, status);
        var result = await _mediator.Send(query);
